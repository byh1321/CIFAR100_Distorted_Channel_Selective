...
if args.mode == 0:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180906_half_clean_15_blur.t0')
	ckpt = torch.load('./checkpoint/ckpt_20180906_half_clean_10_blur.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
		print('==> Resuming from checkpoint..')
...
def train(epoch):
	...
	for batch_idx, (inputs, targets) in enumerate(train_loader):
		...
		outputs = net(inputs)
		'''
		f=open('train_ground_truth.csv','a+')
		for i in range(1000):
			print(targets[i].data[0], file=f)
		f.close()
		'''
...
def test():
	...
	for batch_idx, (inputs, targets) in enumerate(test_loader):
		...
		loss = criterion(outputs, targets)
		'''
		f=open('test_ground_truth.csv','a+')
	...
	if acc > best_acc:
		...
		if args.mode == 0:
			...
			print('Saving..')
			torch.save(state, './checkpoint/ckpt_20180906_half_clean_15_blur.t0')
		best_acc = acc
