...
train_loader = torch.utils.data.DataLoader(torch.utils.data.ConcatDataset([cifar_train, cifar_train_blur_05, cifar_train_gaussian_008_blur_033_mixed]),batch_size=args.bs, shuffle=False,num_workers=8,drop_last=False)
...
if args.mode == 0:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180906_half_clean_05_blur.t0')
	ckpt = torch.load('./checkpoint/ckpt_20180906_half_clean.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
		print('==> Resuming from checkpoint..')
...
def test():
	...
	if acc > best_acc:
		...
		if args.mode == 0:
			...
			print('Saving..')
			torch.save(state, './checkpoint/ckpt_20180906_half_clean_05_blur.t0')
		best_acc = acc
