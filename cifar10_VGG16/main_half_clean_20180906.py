...
train_loader = torch.utils.data.DataLoader(cifar_train,batch_size=args.bs, shuffle=False,num_workers=8,drop_last=False)
...
if args.mode == 0:
	...
	if args.resume:
		print('==> Resuming from checkpoint..')
		assert os.path.isdir('checkpoint'), 'Error: no checkpoint directory found!'
		checkpoint = torch.load('./checkpoint/ckpt_20180906_half_clean.t0')
...
def test():
	...
	if acc > best_acc:
		...
		if args.mode == 0:
			...
			print('Saving..')
			#torch.save(state, './checkpoint/ckpt_20180906_half_clean.t0')
		best_acc = acc
