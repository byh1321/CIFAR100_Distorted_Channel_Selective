...
#test_loader = torch.utils.data.DataLoader(cifar_test_gaussian_025,batch_size=1250, shuffle=False,num_workers=8,drop_last=False)
...
if args.mode == 0:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20190802_half_clean_G3.t0')
	ckpt = torch.load('./checkpoint/ckpt_20190802_half_clean_G2.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
		print('==> Resuming from checkpoint..')
...
def test(net):
	...
	if acc > best_acc:
		...
		if args.mode == 0:
			...
			print('Saving..')
			torch.save(state, './checkpoint/ckpt_20190802_half_clean_G3.t0')
		best_acc = acc
