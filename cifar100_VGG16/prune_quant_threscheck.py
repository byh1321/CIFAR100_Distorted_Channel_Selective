...
def findThreshold(params):
	...
	while 1:
		...
		#result = torch.sum(tmp)/params.size()[0]*64/28 #for full channel
		#result = torch.sum(tmp)/params.size()[0]*64/11 #for 0.25
		result = torch.sum(tmp)/params.size()[0]*64/9 #for 0.125
...
#1
	...
	mask_channel = cn.set_mask(mask_channel, 0, 1)
	mask_channel = cn.set_mask(mask_channel, 2, 0)
	net = cn.net_mask_mul(net, mask_channel)
