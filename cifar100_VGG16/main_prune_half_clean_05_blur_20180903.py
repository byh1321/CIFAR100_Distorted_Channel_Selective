...
parser.add_argument('--network', default='ckpt_20180722_half_clean_06_blur.t0', help='input network ckpt name', metavar="FILE")
...
train_loader = torch.utils.data.DataLoader(torch.utils.data.ConcatDataset([cifar_train, cifar_train_blur_05, cifar_train_gaussian_008_blur_033_mixed]),batch_size=args.bs, shuffle=True,num_workers=8,drop_last=False)
...
if args.mode == 0:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180903_prune_half_clean_05_blur.t0')
	ckpt = torch.load('./checkpoint/ckpt_20180903_prune_half_clean_prune_80_pprec_15.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180903_prune_half_clean_05_blur_prune_80_pprec_15.t0')
	ckpt = torch.load('./checkpoint/ckpt_20180903_prune_half_clean_prune_80_pprec_15.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
