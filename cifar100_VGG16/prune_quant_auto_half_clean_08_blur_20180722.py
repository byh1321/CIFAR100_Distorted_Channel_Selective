...
train_loader = torch.utils.data.DataLoader(torch.utils.data.ConcatDataset([cifar_train_blur_08, cifar_train_gaussian_016_blur_08_mixed]),batch_size=args.bs, shuffle=True,num_workers=8,drop_last=False)
...
if args.mode == 0:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180722_half_clean_08_blur.t0')
	ckpt = torch.load('./checkpoint/ckpt_20180722_half_clean_06_blur_prune_80_pprec_15.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180722_half_clean_08_blur_prune_80_pprec_15.t0')
	ckpt = torch.load('./checkpoint/ckpt_20180722_half_clean_06_blur_prune_80_pprec_15.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
