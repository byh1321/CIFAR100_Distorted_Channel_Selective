...
class CNN(nn.Module):
	...
	def forward(self,x):
		...
		for param in self.fc3[0].parameters():
			param.data = torch.mul(param.data,mask_amp.cuda())
			#print(type(param))
...
def test():
	...
	if acc > best_acc:
		...
		if not os.path.isdir('checkpoint'):
			os.mkdir('checkpoint')
		#torch.save(state,'./checkpoint/ckpt_20180609_half_clean_half_blur_best1.t0')
		best_acc = acc
