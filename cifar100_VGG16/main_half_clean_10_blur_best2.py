...
train_loader = torch.utils.data.DataLoader(torch.utils.data.ConcatDataset([cifar_train_blur_10, cifar_train_gaussian_025, cifar_train_gaussian_025_blur_10_mixed]),batch_size=args.bs, shuffle=True,num_workers=8,drop_last=False)
...
checkpoint = torch.load('./checkpoint/ckpt_20180609_half_clean_10_blur_best2.t0')
ckpt = torch.load('./checkpoint/ckpt_20180609_half_clean_06_blur_best2.t0')
if args.resume == 0:
	best_acc = 0
else:
...
def test():
	...
	if acc > best_acc:
		...
		if not os.path.isdir('checkpoint'):
			os.mkdir('checkpoint')
		torch.save(state,'./checkpoint/ckpt_20180609_half_clean_10_blur_best2.t0')
		best_acc = acc
...
if mode == 1: # mode=1 is training & inference @ each epoch
	for epoch in range(start_epoch, start_epoch+num_epoch):
		...
		#For 0.125 dirty train
		mask = cn.set_mask(cn.set_mask(mask, 0, 1), 3, 0)
		net = cn.net_mask_mul(net, mask)
		net = cn.add_network(net, net2) 
