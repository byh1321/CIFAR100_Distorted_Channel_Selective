...
parser.add_argument('--block', type=int, default=2, metavar='N',help='Choose range of network blocking')
...
train_loader = torch.utils.data.DataLoader(torch.utils.data.ConcatDataset([cifar_train, cifar_train_gaussian_025, cifar_train_blur_10, cifar_train_gaussian_blur_mixed]),batch_size=args.bs, shuffle=True,num_workers=3,drop_last=False)
test_loader = torch.utils.data.DataLoader(cifar_test_blur_10,batch_size=10000, shuffle=False,num_workers=3,drop_last=False)
#train_loader = torch.utils.data.DataLoader(cifar_train,batch_size=args.bs, shuffle=True,num_workers=3,drop_last=False)
...
mask_fc2 = torch.zeros(100,512).cuda()
...
if blocking == 3:
	...
	for i in range(128):
		mask_conv4[i,0:63,:,:] = 1
		mask_conv5[i,0:127,:,:] = 1
		mask_conv6[i,0:127,:,:] = 1
	for i in range(256):
		mask_conv7[i,0:127,:,:] = 1
		mask_conv8[i,0:255,:,:] = 1
		mask_conv9[i,0:255,:,:] = 1
		mask_conv10[i,0:255,:,:] = 1
		mask_conv11[i,0:255,:,:] = 1
		mask_conv12[i,0:255,:,:] = 1
		mask_fc0[i,0:255] = 1 
		mask_fc1[i,0:255] = 1 
	mask_fc2[:,0:255] = 1 
elif blocking == 2:
	...
	for i in range(160):
		mask_conv4[i,0:79,:,:] = 1
		mask_conv5[i,0:159,:,:] = 1
		mask_conv6[i,0:159,:,:] = 1
	for i in range(320):
		mask_conv7[i,0:159,:,:] = 1
		mask_conv8[i,0:319,:,:] = 1
		mask_conv9[i,0:319,:,:] = 1
		mask_conv10[i,0:319,:,:] = 1
		mask_conv11[i,0:319,:,:] = 1
		mask_conv12[i,0:319,:,:] = 1
		mask_fc0[i,0:319] = 1 
		mask_fc1[i,0:319] = 1 
	mask_fc2[:,0:319] = 1 
elif blocking == 1:
	...
	for i in range(112):
		mask_conv2[i,0:55,:,:] = 1 
		mask_conv3[i,0:111,:,:] = 1
	for i in range(224):
		mask_conv4[i,0:111,:,:] = 1
		mask_conv5[i,0:223,:,:] = 1
		mask_conv6[i,0:223,:,:] = 1
	for i in range(448):
		mask_conv7[i,0:223,:,:] = 1
		mask_conv8[i,0:447,:,:] = 1
		mask_conv9[i,0:447,:,:] = 1
		mask_conv10[i,0:447,:,:] = 1
		mask_conv11[i,0:447,:,:] = 1
		mask_conv12[i,0:447,:,:] = 1
		mask_fc0[i,0:447] = 1 
		mask_fc1[i,0:447] = 1 
	mask_fc2[:,0:447] = 1 
...
if args.resume:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180609_0.375_blocked_blur.t0')
	best_acc = 0 
	net = checkpoint['net']
...
if use_cuda:
	net.cuda()
	net = torch.nn.DataParallel(net, device_ids=range(0,3))
...
def test():
	...
	if acc > best_acc:
		...
		if not os.path.isdir('checkpoint'):
			os.mkdir('checkpoint')
		torch.save(state, './checkpoint/ckpt_20180609_0.375_blocked_blur.t0')
		best_acc = acc
...
def block_network():
	...
	for child in net.children():
		for param in child.conv2[0].parameters():
			param.grad.data = torch.mul(param.grad.data, mask_conv1)
			param.data = torch.mul(param.data,mask_conv1)
	for child in net.children():
		for param in child.conv3[0].parameters():
			param.grad.data = torch.mul(param.grad.data, mask_conv2)
			param.data = torch.mul(param.data,mask_conv2)
	for child in net.children():
	...
	for child in net.children():
		for param in child.conv5[0].parameters():
			param.grad.data = torch.mul(param.grad.data, mask_conv4)
			param.data = torch.mul(param.data,mask_conv4)
	for child in net.children():
		for param in child.conv6[0].parameters():
			param.grad.data = torch.mul(param.grad.data, mask_conv5)
			param.data = torch.mul(param.data,mask_conv5)
	for child in net.children():
		for param in child.conv7[0].parameters():
			param.grad.data = torch.mul(param.grad.data, mask_conv6)
			param.data = torch.mul(param.data,mask_conv6)
	for child in net.children():
	...
	for child in net.children():
		for param in child.conv9[0].parameters():
			param.grad.data = torch.mul(param.grad.data, mask_conv8)
			param.data = torch.mul(param.data,mask_conv8)
	for child in net.children():
		for param in child.conv10[0].parameters():
			param.grad.data = torch.mul(param.grad.data, mask_conv9)
			param.data = torch.mul(param.data,mask_conv9)
	for child in net.children():
	...
	for child in net.children():
		for param in child.conv12[0].parameters():
			param.grad.data = torch.mul(param.grad.data, mask_conv11)
			param.data = torch.mul(param.data,mask_conv11)
	for child in net.children():
		for param in child.conv13[0].parameters():
			param.grad.data = torch.mul(param.grad.data, mask_conv12)
			param.data = torch.mul(param.data,mask_conv12)

	for child in net.children():
		for param in child.fc1[1].parameters():
			param.grad.data = torch.mul(param.grad.data, mask_fc0)
			param.data = torch.mul(param.data,mask_fc0)
	for child in net.children():
	...
	for child in net.children():
		for param in child.fc3[0].parameters():
			param.grad.data = torch.mul(param.grad.data, mask_fc2)
			param.data = torch.mul(param.data,mask_fc2)
