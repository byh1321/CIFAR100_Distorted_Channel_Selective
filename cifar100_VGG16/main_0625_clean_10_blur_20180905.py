...
if args.mode == 0:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180905_0625_clean_10_blur.t0')
	ckpt = torch.load('./checkpoint/ckpt_20180905_0625_clean_05_blur.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
		print('==> Resuming from checkpoint..')
...
def train(epoch):
	...
	mask_channel = set_mask(set_mask(mask_channel, 1, 1), 2, 0)
...
def test():
	...
	if acc > best_acc:
		...
		if args.mode == 0:
			...
			print('Saving..')
			torch.save(state, './checkpoint/ckpt_20180905_0625_clean_10_blur.t0')
		best_acc = acc
