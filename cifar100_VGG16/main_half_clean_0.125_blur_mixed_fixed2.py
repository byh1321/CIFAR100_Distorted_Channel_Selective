...
train_loader = torch.utils.data.DataLoader(torch.utils.data.ConcatDataset([cifar_train_blur_03, cifar_train_gaussian_008_blur_03_mixed]),batch_size=args.bs, shuffle=True,num_workers=8,drop_last=False)
...
checkpoint = torch.load('./checkpoint/ckpt_20180609_half_clean_0.125_blur_mixed_fixed2.t0')
...
def test():
	...
	if acc > best_acc:
		...
		if not os.path.isdir('checkpoint'):
			os.mkdir('checkpoint')
		torch.save(state,'./checkpoint/ckpt_20180609_half_clean_0.125_blur_mixed_fixed2.t0')
		best_acc = acc
