...
if args.mode == 0:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180716_half_clean_016_gaussian.t0')
	ckpt = torch.load('./checkpoint/ckpt_20180716_half_clean_008_gaussian_prune_80_pprec_15_69.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
		print('==> Resuming from checkpoint..')
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180716_half_clean_016_gaussian_prune_80_pprec_15_69.t0')
	ckpt = torch.load('./checkpoint/ckpt_20180716_half_clean_008_gaussian_prune_80_pprec_15_69.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
...
def test():
	...
	if acc > best_acc:
		...
		if args.mode == 0:
			...
			print('Saving..')
			torch.save(state,'./checkpoint/ckpt_20180716_half_clean_016_gaussian.t0')
		elif args.mode == 2:
			print('Saving..')
			torch.save(state,'./checkpoint/ckpt_20180716_half_clean_016_gaussian_prune_80_pprec_15_69.t0')
		best_acc = acc
...
if mode == 0: # only inference
	...
	for epoch in range(start_epoch, start_epoch+num_epoch):
		...
		mask_channel = cn.set_mask(cn.set_mask(mask_channel, 2, 1), 3, 0)
