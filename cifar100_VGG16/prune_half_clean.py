...
if args.resume:
	checkpoint = torch.load('./checkpoint/ckpt_20180710_half_clean_prune_80.t0')
...
def test():
	...
	if acc > best_acc:
		...
		if not os.path.isdir('checkpoint'):
			os.mkdir('checkpoint')
		torch.save(state, './checkpoint/ckpt_20180710_half_clean_prune_80.t0')
		best_acc = acc
...
def retrain(epoch,mask_conv0,mask_conv3,mask_conv7,mask_conv10,mask_conv14,mask_conv17,mask_conv20,mask_conv24,mask_conv27,mask_conv30,mask_conv34,mask_conv37,mask_conv40,mask_fc1,mask_fc4,mask_fc6):
	...
	correct = 0
	try:
		mask = torch.load('mask_{}_half_clean.dat'.format(args.pr))
...
if prune:
	if not(os.path.isfile('./mask_{}_half_clean.dat'.format(args.pr))):
