...
#train_loader = torch.utils.data.DataLoader(torch.utils.data.ConcatDataset([cifar_train, cifar_train_blur_033, cifar_train_gaussian_008_blur_033_mixed]),batch_size=args.bs, shuffle=True,num_workers=8,drop_last=False)
...
test_loader = torch.utils.data.DataLoader(cifar_test_blur_033,batch_size=10000, shuffle=False,num_workers=8,drop_last=False)
...
if args.mode == 0:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180906_half_clean_033_blur.t0')
...
def test():
	...
	if acc > best_acc:
		...
		if args.mode == 0:
			...
			print('Saving..')
			torch.save(state, './checkpoint/ckpt_20180906_half_clean_033_blur.t0')
		best_acc = acc
