...
parser.add_argument('--network', default='ckpt_20180722_half_clean_10_blur.t0', help='input network ckpt name', metavar="FILE")
...
test_loader = torch.utils.data.DataLoader(cifar_test_blur_15,batch_size=10000, shuffle=False,num_workers=8,drop_last=False)
...
if args.mode == 0:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180903_prune_half_clean_15_blur.t0')
	ckpt = torch.load('./checkpoint/ckpt_20180903_prune_half_clean_10_blur_prune_80_pprec_15.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180903_prune_half_clean_15_blur_prune_80_pprec_15.t0')
	ckpt = torch.load('./checkpoint/ckpt_20180903_prune_half_clean_10_blur_prune_80_pprec_15.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
