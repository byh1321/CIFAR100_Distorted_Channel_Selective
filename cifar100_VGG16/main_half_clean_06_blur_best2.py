...
train_loader = torch.utils.data.DataLoader(torch.utils.data.ConcatDataset([cifar_train_blur_06, cifar_train_gaussian_016, cifar_train_gaussian_016_blur_06_mixed]),batch_size=args.bs, shuffle=True,num_workers=8,drop_last=False)
...
checkpoint = torch.load('./checkpoint/ckpt_20180609_half_clean_06_blur_best2.t0')
...
def test():
	...
	if acc > best_acc:
		...
		if not os.path.isdir('checkpoint'):
			os.mkdir('checkpoint')
		torch.save(state,'./checkpoint/ckpt_20180609_half_clean_06_blur_best2.t0')
		best_acc = acc
