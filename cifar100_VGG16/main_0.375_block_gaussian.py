...
train_loader = torch.utils.data.DataLoader(torch.utils.data.ConcatDataset([cifar_train, cifar_train_gaussian_025]),batch_size=args.bs, shuffle=True,num_workers=3,drop_last=False)
test_loader = torch.utils.data.DataLoader(cifar_test_gaussian_025,batch_size=10000, shuffle=False,num_workers=3,drop_last=False)
...
if args.resume:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180609_0.375_blocked_gaussian.t0')
	best_acc = 0 
	net = checkpoint['net']
...
def test():
	...
	if acc > best_acc:
		...
		if not os.path.isdir('checkpoint'):
			os.mkdir('checkpoint')
		torch.save(state, './checkpoint/ckpt_20180609_0.375_blocked_gaussian.t0')
		best_acc = acc
