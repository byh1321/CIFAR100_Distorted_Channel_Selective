...
train_loader = torch.utils.data.DataLoader(cifar_train_gaussian_016,batch_size=args.bs, shuffle=True,num_workers=8,drop_last=False)
...
checkpoint = torch.load('./checkpoint/ckpt_20180609_half_clean_0.25_gaussian_fixed.t0')
ckpt = torch.load('./checkpoint/ckpt_20180609_half_clean_0.125_gaussian_fixed.t0')
...
def test():
	...
	if acc > best_acc:
		...
		if not os.path.isdir('checkpoint'):
			os.mkdir('checkpoint')
		torch.save(state,'./checkpoint/ckpt_20180609_half_clean_0.25_gaussian_fixed.t0')
		best_acc = acc
