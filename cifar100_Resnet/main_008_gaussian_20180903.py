...
def save_network(layer):
	for child in net2.children():
		for param in child.conv1[0].parameters():
			layer[0] = param.data
	for child in net2.children():
		for param in child.layer1_basic_1[0].parameters():
			layer[1] = param.data		
	for child in net2.children():
		for param in child.layer1_basic_2[0].parameters():
			layer[2] = param.data		
	for child in net2.children():
		for param in child.layer1_basic_3[0].parameters():
			layer[3] = param.data		
	for child in net2.children():
		for param in child.layer1_basic_4[0].parameters():
			layer[4] = param.data	
	for child in net2.children():
		for param in child.layer2_basic_1[0].parameters():
			layer[5] = param.data
	for child in net2.children():
		for param in child.layer2_basic_2[0].parameters():
			layer[6] = param.data
	for child in net2.children():
		for param in child.layer2_basic_3[0].parameters():
			layer[7] = param.data
	for child in net2.children():
		for param in child.layer2_basic_4[0].parameters():
			layer[8] = param.data
	for child in net2.children():
		for param in child.layer3_basic_1[0].parameters():
			layer[9] = param.data
	for child in net2.children():
		for param in child.layer3_basic_2[0].parameters():
			layer[10] = param.data
	for child in net2.children():
		for param in child.layer3_basic_3[0].parameters():
			layer[11] = param.data
	for child in net2.children():
		for param in child.layer3_basic_4[0].parameters():
			layer[12] = param.data
	for child in net2.children():
		for param in child.layer4_basic_1[0].parameters():
			layer[13] = param.data
	for child in net2.children():
		for param in child.layer4_basic_2[0].parameters():
			layer[14] = param.data
	for child in net2.children():
		for param in child.layer4_basic_3[0].parameters():
			layer[15] = param.data
	for child in net2.children():
		for param in child.layer4_basic_4[0].parameters():
			layer[16] = param.data
	for child in net2.children():
...
def add_network():
	...
	for child in net.children():
		for param in child.conv1[0].parameters():
			param.data[0] = torch.add(param.data,layer[0])
	for child in net.children():
		for param in child.layer1_basic_1[0].parameters():
			param.data[1] = torch.add(param.data,layer[1])		
	for child in net.children():
		for param in child.layer1_basic_2[0].parameters():
			param.data[2] = torch.add(param.data,layer[2])		
	for child in net.children():
		for param in child.layer1_basic_3[0].parameters():
			param.data[3] = torch.add(param.data,layer[3])		
	for child in net.children():
		for param in child.layer1_basic_4[0].parameters():
			param.data[4] = torch.add(param.data,layer[4])	
	for child in net.children():
		for param in child.layer2_basic_1[0].parameters():
			param.data[5] = torch.add(param.data,layer[5])
	for child in net.children():
		for param in child.layer2_basic_2[0].parameters():
			param.data[6] = torch.add(param.data,layer[6])
	for child in net.children():
		for param in child.layer2_basic_3[0].parameters():
			param.data[7] = torch.add(param.data,layer[7])
	for child in net.children():
		for param in child.layer2_basic_4[0].parameters():
			param.data[8] = torch.add(param.data,layer[8])
	for child in net.children():
		for param in child.layer3_basic_1[0].parameters():
			param.data[9] = torch.add(param.data,layer[9])
	for child in net.children():
		for param in child.layer3_basic_2[0].parameters():
			param.data[10] = torch.add(param.data,layer[10])
	for child in net.children():
		for param in child.layer3_basic_3[0].parameters():
			param.data[11] = torch.add(param.data,layer[11])
	for child in net.children():
		for param in child.layer3_basic_4[0].parameters():
			param.data[12] = torch.add(param.data,layer[12])
	for child in net.children():
		for param in child.layer4_basic_1[0].parameters():
			param.data[13] = torch.add(param.data,layer[13])
	for child in net.children():
		for param in child.layer4_basic_2[0].parameters():
			param.data[14] = torch.add(param.data,layer[14])
	for child in net.children():
		for param in child.layer4_basic_3[0].parameters():
			param.data[15] = torch.add(param.data,layer[15])
	for child in net.children():
		for param in child.layer4_basic_4[0].parameters():
			param.data[16] = torch.add(param.data,layer[16])
	for child in net.children():
		for param in child.layer2_downsample[0].parameters():
			param.data[17] = torch.add(param.data,layer[17])
	for child in net.children():
		for param in child.layer3_downsample[0].parameters():
			param.data[18] = torch.add(param.data,layer[18])
	for child in net.children():
		for param in child.layer4_downsample[0].parameters():
			param.data[19] = torch.add(param.data,layer[19])
	for child in net.children():
		for param in child.linear[0].parameters():
			param.data[20] = torch.add(param.data,layer[20])
	return layer
	...
	if acc > best_acc:
		if args.mode == 0:
			...
			if not os.path.isdir('checkpoint'):
				os.mkdir('checkpoint')
			torch.save(state, './checkpoint/ckpt_20180903.t0')
			best_acc = acc
