...
train_loader = torch.utils.data.DataLoader(train_dataset,batch_size=1, shuffle=False,num_workers=8,drop_last=False)
...
def imshow(img,i,size):
	...
	if args.val:
		foldername = '/home/Imagenet2012/dirty/imagenet_gaussian_{}_blur_{}_val'.format(args.gaussian_std, args.blur_std)
	...
	if args.train:
		foldername = '/home/Imagenet2012/dirty/imagenet_gaussian_{}_blur_{}_train'.format(args.gaussian_std, args.blur_std)
		if not os.path.isdir(foldername):
			os.mkdir(foldername)
			assert os.path.isdir(foldername), 'Error: no checkpoint directory found!'
		scipy.misc.toimage(np.transpose(npimg, (1,2,0))).save(foldername+"/imagenet_gaussian_{}_blur_{}_train_{i}.png".format(args.gaussian_std,args.blur_std,i=i))
