...
parser.add_argument('--network', default='ckpt_dump.t0', help='input network ckpt name', metavar="FILE")
...
if args.mode == 0:
	...
	if args.resume:
		...
		checkpoint = torch.load('./checkpoint/ckpt_20190429.t0')
		net2 = checkpoint['net']
		net = VGG16()
		#net = nn.module.load_state_dict(net2.module.state_dict())
	...
	if top1.avg > top1_acc:
		if mode == 0:
			...
			if not os.path.isdir('checkpoint'):
				os.mkdir('checkpoint')
			torch.save(state, './checkpoint/'+args.network)
			top1_acc = top1.avg
