...
parser.add_argument('--network', default='ckpt_20190103_half_clean_G2.t0', help='input network ckpt name', metavar="FILE")
...
if args.mode == 0:
	...
	if args.resume:
		...
		checkpoint = torch.load('./checkpoint/ckpt_20190103_half_clean_G2.t0')
		net = checkpoint['net']
		ckpt = torch.load('./checkpoint/ckpt_20190103_half_clean_G1.t0')
		net2 = ckpt['net']
	...
	if top1.avg > top1_acc:
		if mode == 0:
			...
			if not os.path.isdir('checkpoint'):
				os.mkdir('checkpoint')
			torch.save(state, './checkpoint/ckpt_20190103_half_clean_G2.t0')
			top1_acc = top1.avg
