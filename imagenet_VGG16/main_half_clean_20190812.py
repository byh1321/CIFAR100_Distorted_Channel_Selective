...
parser.add_argument('--prindex', default=0, type=float)
...
if args.mode == 0:
	...
	if args.resume:
		...
		checkpoint = torch.load('./checkpoint/ckpt_20190103_half_clean.t0')
		net = checkpoint['net']
	...
	if top1.avg > top1_acc:
		if mode == 0:
			...
			if not os.path.isdir('checkpoint'):
				os.mkdir('checkpoint')
			torch.save(state, './checkpoint/ckpt_20190103_half_clean.t0')
			top1_acc = top1.avg
