...
parser.add_argument('--network', default='ckpt_20180820_half_clean.t0', help='input network ckpt name', metavar="FILE")
...
if args.mode == 0:
	...
	if args.resume:
		...
		checkpoint = torch.load('./checkpoint/ckpt_20180820_half_clean.t0')
		net = checkpoint['net']
	...
	if top1.avg > top1_acc:
		if mode == 0:
			...
			if not os.path.isdir('checkpoint'):
				os.mkdir('checkpoint')
			torch.save(state, './checkpoint/ckpt_20180820_half_clean.t0')
			top1_acc = top1.avg
