...
if args.mode == 0:
	...
	checkpoint = torch.load('./checkpoint/ckpt_20180913_half_clean_G3.t0')
	net = checkpoint['net']

elif args.mode == 1:
	checkpoint = torch.load('./checkpoint/ckpt_20180913_half_clean_G3.t0')
	ckpt = torch.load('./checkpoint/ckpt_20180913_half_clean_G2.t0')
	net = checkpoint['net']
	net2 = ckpt['net']
	if args.resume:
		print('==> Resuming from checkpoint..')
	...
	if acc > best_acc:
		if args.mode == 0:
			...
			if not os.path.isdir('checkpoint'):
				os.mkdir('checkpoint')
			torch.save(state, './checkpoint/ckpt_20180913_half_clean_G3.t0')
			best_acc = acc
